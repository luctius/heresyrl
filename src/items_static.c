...
#define AVERAGE_STUB_REVOLVER_DESC "This a generic stub revolver."
...
#define CREATURE_MELEE(wpn_cat,dmg_die,dmg_add,_dmg_type,_penetration,special) .icon=' ',.stacked_quantity=0, .max_quantity=1,\
            ...
            .penetration=_penetration, .special_quality=special ,}

#define MELEE(wpn_cat,dmg_die,dmg_add,_dmg_type,_penetration,_upgrades,special) .icon='|',.stacked_quantity=0, .max_quantity=1,\
            ...
            .special_quality=special .upgrades=_upgrades ,}

#define PISTOL(_dmg_type,dmg_die,dmg_add,_range,rof_single,rof_semi,rof_auto,mag_sz,_penetration,_upgrades,special) \
            ...
            .ammo_type=AMMO_TYPE_PISTOL, .ammo_used_template_id=ITEM_ID_AVERAGE_PISTOL_AMMO, .special_quality=special, \
            .upgrades=_upgrades, .jammed=false,}
...
struct itm_item static_item_list[] = {
    ...
    ITEM(ITEM_ID_FIXED_LIGHT,    "torch",    "a torch",   AVERAGE_TORCH_DESC,  ITEM_AVAILABILITY_AVERAGE, ITEM_QUALITY_AVERAGE, 1,     1,   1),
        /*        tool type     energy  luminem*/
        LIGHT(TOOL_TYPE_LIGHT,  65000,     12), ITEM_END,

    ITEM(ITEM_ID_AVERAGE_TORCH,  "torch",    "a torch",   AVERAGE_TORCH_DESC,  ITEM_AVAILABILITY_AVERAGE, ITEM_QUALITY_AVERAGE, 1,     1,   1),
    ...
    ITEM(ITEM_ID_HUMAN_UNARMED,  "hands",    "hands",     "",                  0,                         0,                    0,    0,   0),
        /*                 CATEGORY             xd10  +X     dmg type              upgrades       special qualities */
        CREATURE_MELEE(WEAPON_CATEGORY_2H_MELEE,0,    -3,  WEAPON_DMG_TYPE_IMPACT, 0,         WEAPON_SPEC_QUALITY_UNARMED), ITEM_END,

    /* Melee */

    /* Pistols */
    /*    ID                            short name         long name          description           availability            quality                   weight,cost,delay*/
    ITEM(ITEM_ID_AVERAGE_STUB_AUTOMATIC,"stub automatic","a stub automatic",AVERAGE_STUB_AUTOMATIC_DESC,ITEM_AVAILABILITY_AVERAGE,ITEM_QUALITY_AVERAGE, 15,    50,  1),
        /*       dmg type             xd10  +x   range  single semi  auto  mag_sz  pen  upgrades  special*/
        PISTOL(WEAPON_DMG_TYPE_IMPACT,1,     3,  30,     1,    3,    0,    9,      0,   0,        0), ITEM_END,

    ITEM(ITEM_ID_AVERAGE_STUB_REVOLVER,"stub revolver",  "a stub revolver", AVERAGE_STUB_REVOLVER_DESC, ITEM_AVAILABILITY_AVERAGE,ITEM_QUALITY_AVERAGE, 15,    50,  2),
        PISTOL(WEAPON_DMG_TYPE_IMPACT,1,     3,  30,     1,    3,    0,    6,      0,   0,        WEAPON_SPEC_QUALITY_RELIABLE), ITEM_END,

    /* Ammo */
    /*    ID                          short name    long name           description    unvailability       quality           weight,cost,delay*/
    ITEM(ITEM_ID_AVERAGE_PISTOL_AMMO,"pistol ammo","A clip of pistol ammo","",ITEM_AVAILABILITY_AVERAGE,ITEM_QUALITY_AVERAGE,0,     1,   0),
